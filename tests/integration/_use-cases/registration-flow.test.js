...
describe("Use case: Registration Flow (all successful)", () => {
  ...
  test("Receive activation email", async () => {
    ...
    expect(activationEmail.subject).toBe(
      "Ative seu cadastro no Clone do TabNews!",
    );
    ...
    expect(activationEmail.text).toContain(
  ...
  test("Activate account", async () => {
    const activationResponse = await fetch(
      `http://localhost:3000/api/v1/activations/${tokenUUID}`,
      {
        method: "PATCH",
      },
    );
